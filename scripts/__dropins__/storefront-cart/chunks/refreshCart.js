...
`,hr=`
  ...
  ${vr}
  ${A}
`,G=async()=>{const r=o.authenticated,n=o.cartId;if(r)return I(hr,{method:"POST"}).then(({errors:c,data:e})=>{if(c)return U(c);const u={...e.cart,...e.customer};return k(u)});if(!n)throw new Error("No cart ID found");return I(Cr,{method:"POST",cache:"no-cache",variables:{cartId:n}}).then(({errors:c,data:e})=>c?U(c):k(e.cart))},br=`
  ...
  ${A}
`,D=async()=>{if(o.initializing)return null;o.initializing=!0,o.config||(o.config=await Tr());const r=o.authenticated?await Q():await W();return g.emit("cart/initialized",r),g.emit("cart/data",r),o.initializing=!1,r};async function Q(){const r=o.cartId,n=await G();return n?(o.cartId=n.id,!r||n.id===r?n:await I(br,{variables:{guestCartId:r,customerCartId:n.id}}).then(()=>G()).then(c=>{const e={oldCartItems:n.items,newCart:c};return g.emit("cart/merged",e),c}).catch(()=>(console.error("Could not merge carts"),n))):null}async function W(){if(B.getConfig().disableGuestCart===!0||!o.cartId)return null;try{return await G()}catch(r){return console.error(r),null}}const xr=`
...
`,Tr=async()=>I(xr,{method:"GET",cache:"force-cache"}).then(({errors:r,data:n})=>r?U(r):mr(n.storeConfig)),z=async()=>{const r=o.authenticated?await Q():await W();return g.emit("cart/data",r),r};export{D as a,Q as b,B as c,W as d,Tr as e,G as g,q as i,z as r,k as t};
